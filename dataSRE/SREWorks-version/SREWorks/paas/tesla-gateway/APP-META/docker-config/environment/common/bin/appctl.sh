...
start() {
    ...
    sudo systemctl preset node-exporter
...
start_pandora_boot() {
    ...
    do
        ...
        if [ -z "$ret" ]; then
            ...
            if [ $ret -ne 0 ]; then
                echo "INFO: waiting route load sleep 10s"
                sleep 10
                echo "online : http://127.0.0.1:7001/online?token=teslaAdmin"
                curl "http://127.0.0.1:7001/online?token=teslaAdmin"
                echo $?
...
stop() {
    ...
    lsof -i:7001
    if [ $? -eq 0 ]; then
      echo "http://127.0.0.1:7001/offline"
      curl "http://127.0.0.1:7001/offline?token=teslaAdmin"
      sleep 30
    fi
...
start_nginx() {
    ...
    if [ $ret -ne 0 -a "${NGINX_SKIP}" -ne "1" ]; then
        ...
        if test -n "${STATUS_PORT}"; then
            ...
            echo "check http://127.0.0.1:7001/status.taobao success"
        fi
