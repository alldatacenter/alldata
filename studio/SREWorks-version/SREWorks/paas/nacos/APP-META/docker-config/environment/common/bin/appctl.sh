...
start_pandora_boot() {
    ...
    ## 预发单机，使用单机模式部署
    export NACOS_OPT='-Dnacos.standalone=false'
    if [ "${ali_env_sign}" = staging ]; then
      NACOS_OPT='-Dnacos.standalone=true'
    fi
...
start_nginx() {
    ...
    if [ $ret -ne 0 -a "${NGINX_SKIP}" -ne "1" ]; then
        ...
        if test -n "${STATUS_PORT}"; then
            ...
            status_code=`/usr/bin/curl -L -o /dev/null --connect-timeout 5 -s -w %{http_code}  "http://127.0.0.1:${STATUS_PORT}/nacos/status.taobao"`
            if [ x$status_code != x200 ];then
                echo "check http://127.0.0.1:${STATUS_PORT}/nacos/status.taobao failed with status ${status_code} after wait ${STATUS_TAOBAO_WAIT_TIME} seconds."
            ...
            echo "check http://127.0.0.1:${STATUS_PORT}/nacos/status.taobao success"
        fi
