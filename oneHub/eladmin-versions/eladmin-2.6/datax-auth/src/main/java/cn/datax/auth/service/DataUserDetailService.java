...
import cn.datax.service.system.api.dto.RoleSmallDto;
...
public class DataUserDetailService implements UserDetailsService {
    ...
    @Override
    public DataUser loadUserByUsername(String s) throws UsernameNotFoundException {
        // 远程获取用户
        ...
        UserDto userVo = userInfo.getUserVo();
        Set<RoleSmallDto> roles = userVo.getRoles();
        if (CollUtil.isNotEmpty(roles)) {
            roles.stream()
                    .filter(roleVo -> roleVo.getId()!=0)
                    .forEach(roleVo -> authsSet.add(DataConstant.Security.ROLEPREFIX.getVal() + roleVo.getId()));
        }
