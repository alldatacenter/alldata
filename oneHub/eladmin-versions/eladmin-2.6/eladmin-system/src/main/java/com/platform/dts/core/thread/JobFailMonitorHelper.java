 ...
 *
 * @author AllDataDC 2022/11/1 18:05:56
 */
			...
			public void run() {
				...
				while (!toStop) {
					try {
						...
						if (!toStop) {
							logger.error(">>>>>>>>>>> Eladmin DTS, job fail monitor thread error:{0}", e);
						}
					}
				...
				logger.info(">>>>>>>>>>> Eladmin DTS, job fail monitor thread stop");

			}
		});
		monitorThread.setDaemon(true);
		monitorThread.setName("Eladmin DTS, admin JobFailMonitorHelper");
		monitorThread.start();
	}
		...
		if (info!=null && info.getAlarmEmail()!=null && info.getAlarmEmail().trim().length()>0) {
			...
			for (String email: emailSet) {
				...
				try {
					...
					logger.error(">>>>>>>>>>> Eladmin DTS, job fail alarm email send error, JobLogId:{}", jobLog.getId(), e);
