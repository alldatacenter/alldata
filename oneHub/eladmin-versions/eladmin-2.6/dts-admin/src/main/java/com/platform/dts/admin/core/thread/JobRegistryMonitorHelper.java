...
import com.platform.dts.core.enums.RegistryConfig;
 ...
 * @author AllDataDC 2022/11/02 19:10:24
 */
		...
		registryThread = new Thread(() -> {
			while (!toStop) {
				try {
					...
					if (!toStop) {
						logger.error(">>>>>>>>>>> Eladmin DTS, job registry monitor thread error:{}", e);
					}
				}
				try {
			...
			logger.info(">>>>>>>>>>> Eladmin DTS, job registry monitor thread stop");
		});
		registryThread.setDaemon(true);
		registryThread.setName("Eladmin DTS, admin JobRegistryMonitorHelper");
