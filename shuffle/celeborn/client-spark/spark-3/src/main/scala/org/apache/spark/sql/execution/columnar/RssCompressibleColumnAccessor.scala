...
trait RssCompressibleColumnAccessor[T <: AtomicType] extends RssColumnAccessor {
  this: RssNativeColumnAccessor[T] =>

  private var decoder: Decoder[T] = _

  abstract override protected def initialize(): Unit = {
    super.initialize()
    decoder = RssCompressionScheme(underlyingBuffer.getInt()).decoder(buffer, columnType)
  }
