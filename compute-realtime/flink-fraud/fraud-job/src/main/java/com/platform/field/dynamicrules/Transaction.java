...
public class Transaction implements TimestampAssignable<Long> {
  ...
  private Long ingestionTimestamp;private static transient DateTimeFormatter timeFormatter =
      DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")
          .withLocale(Locale.US)
          .withZone(ZoneOffset.UTC);public enum PaymentType {
    CSH("CSH"),
    CRD("CRD");String representation;PaymentType(String repr) {
    ...
    if (tokens.size() != numArgs) {
      throw new RuntimeException(
          "Invalid transaction: "
              ...
              + " found "
              + tokens.size());
    }Transaction transaction = new Transaction();try {
