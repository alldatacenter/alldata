  ...
  <admin-layout>
    ...
    <div :class="['tabs-view-content', layout, pageWidth]" :style="`margin-top: ${multiPage ? -0 : 0}px`">
      <page-toggle-transition :disabled="animate.disabled" :animate="animate.name" :direction="animate.direction">
        <a-keep-alive :exclude-keys="excludeKeys" v-if="multiPage && cachePage && !$route.meta.notAlive" v-model="clearCaches">
          <router-view v-if="!refreshing" ref="tabContent" :key="$route.path" />
        </a-keep-alive>
        <router-view ref="tabContent" v-else-if="!refreshing" :key="$route.path" />
...
export default {
  ...
  data () {
    return {
      ...
      menuVisible: false,
      refreshing: false,
      excludeKeys: ['/colony-manage', "/security-center/user"]
    }
  },
  computed: {
