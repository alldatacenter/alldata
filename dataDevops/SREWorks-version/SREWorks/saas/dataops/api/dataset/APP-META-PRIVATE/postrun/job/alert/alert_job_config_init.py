...
alert_def_name = "用户下单失败"
incident_def_name = "订单服务异常"
...
def load_job_configs():
    with open(config_path + "/alert_job_config.json", 'r') as load_f:
        job_configs = json.load(load_f)

    for job_config in job_configs:
        event_conf = job_config.get("eventConf", [])
        if event_conf:
            alert_def_id = get_app_oem_def("alert", alert_def_name)
            if alert_def_id is None:
                print("分析任务需要订阅告警TOPIC,但告警TOPIC配置失败")
                return None
            event_conf[0]["config"]["topics"] = ["sreworks-health-alert-" + str(alert_def_id)]
        ...
        for task in task_list:
            ...
            if model_id is not None:
                incident_def_id = get_app_oem_def("incident", incident_def_name)
                if incident_def_id is None:
                    print("分析任务需要关联异常定义,但异常定义配置失败")
                    return None
                else:
                    scene_conf["modelId"] = incident_def_id
