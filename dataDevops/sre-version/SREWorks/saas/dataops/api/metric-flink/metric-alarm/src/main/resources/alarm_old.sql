...
SELECT
  ...
  E.`time`
FROM  metric_filter,
      lateral table (durationAlarm(rule_id,metric_name,`timestamp`,metric_value,tags, granularity))
        as E (service,source,tag,text,title,type,`time`);

----------------------------
CREATE VIEW noDataAlarm_view(eventArray) AS
SELECT
  noDataAlarm(rule_id, `timestamp`)
FROM
  metric_filter
GROUP BY --当上游数据不均匀的时候, 可能会造成window结束不了, 没有结果输出
  TUMBLE (ts, INTERVAL '1' SECOND);-- 上游agg的数据, 在一个窗口内必须都是相同秒数上的
...
FROM  noDataAlarm_view as N,
  lateral table (splitEventList(N.eventArray)) as E (service,source,tag,text,title,type,`time`);
