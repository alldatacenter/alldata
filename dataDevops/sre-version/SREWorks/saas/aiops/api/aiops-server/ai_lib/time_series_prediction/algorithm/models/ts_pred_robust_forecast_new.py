# -*- coding: utf-8 -*-
# @author: 丛戎
# @target: 新版预测模型，基于stl和arima预测算法，同时改用scikit-learn的API格式
...
class TdataRobustPredNewModel(BaseEstimator):
    """
    调用达摩院自研预测算法，非周期部分运用ses进行预测
    """
    ...
    def fit(self, X, y=None):
        ...
        if passed_check_acf:
            ...
            # 计算上下界
            # std = np.std(np.array(decomp_result['residual']))
            ...
            # 增加上下界, 即使没有周期性也进行分解
            try:
                decomp_result = DataPreprocessUtils().stl_decomp(kpidata_train, 'y', self.period)
                sigma = decomp_result['remainder'].mad()
            except Exception:
                # 如果不足两天的数据，则令upper = pred
                sigma = 0
            # sigma = kpidata_train["y"].mad()
