 ...
 * and infers the actual memory layout (32 bit vs. 64 bit word size, compressed object pointers vs. uncompressed) from
 * the best available indicators. It can reliably detect a 32 bit vs. 64 bit JVM. It can only make an educated guess
 * at whether compressed OOPs are used, though; specifically, it knows what the JVM's default choice of OOP compression
 * would be based on HotSpot version and maximum heap sizes, but if the choice is explicitly overridden with the
...
public class ObjectSizeCalculator {
   ...
   *            extension, all other Class objects loaded by the same loader, and all the parent class loaders).
   *            It doesn't provide the size of the static fields in the JVM class that the Class object represents.
  ...
  public static long getObjectSize(Object obj) throws UnsupportedOperationException {
    ...
    // especially when the `isAccessible` is false. More details in JEP 403. While integrating Arctic with other
