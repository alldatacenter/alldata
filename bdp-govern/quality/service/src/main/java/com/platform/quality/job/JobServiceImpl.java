...
import static com.platform.quality.job.entity.LivySessionStates.State.BUSY;
import static com.platform.quality.job.entity.LivySessionStates.State.DEAD;
import static com.platform.quality.job.entity.LivySessionStates.State.IDLE;
import static com.platform.quality.job.entity.LivySessionStates.State.NOT_STARTED;
import static com.platform.quality.job.entity.LivySessionStates.State.RECOVERING;
import static com.platform.quality.job.entity.LivySessionStates.State.RUNNING;
import static com.platform.quality.job.entity.LivySessionStates.State.STARTING;
import static com.platform.quality.job.entity.LivySessionStates.State.SUCCESS;
import static com.platform.quality.job.entity.LivySessionStates.State.UNKNOWN;
import static com.platform.quality.job.entity.LivySessionStates.isActive;
import static com.platform.quality.measure.entity.GriffinMeasure.ProcessType.BATCH;
import static com.platform.quality.measure.entity.GriffinMeasure.ProcessType.STREAMING;
...
import com.platform.quality.config.EnvConfig;
import com.platform.quality.event.GriffinEventManager;
import com.platform.quality.event.JobEvent;
...
import com.platform.quality.util.YarnNetUtil;
...
import com.platform.quality.job.entity.JobType;
import com.platform.quality.job.entity.LivySessionStates;
import com.platform.quality.job.entity.LivySessionStates.State;
import com.platform.quality.job.entity.StreamingJob;
import com.platform.quality.job.repo.BatchJobRepo;
import com.platform.quality.job.repo.JobInstanceRepo;
import com.platform.quality.job.repo.JobRepo;
import com.platform.quality.job.repo.StreamingJobRepo;
import com.platform.quality.measure.entity.GriffinMeasure;
import com.platform.quality.measure.entity.GriffinMeasure.ProcessType;
import com.platform.quality.measure.repo.GriffinMeasureRepo;
...
public class JobServiceImpl implements JobService {
    ...
    public AbstractJob getJobConfig(Long jobId) {
        ...
        if (job == null) {
            ...
            throw new GriffinException
                .NotFoundException(GriffinExceptionMessage.JOB_ID_DOES_NOT_EXIST);
        }
        return job;
    }
    ...
    private void doAction(String action, AbstractJob job, JobOperator op)
        ...
        switch (action) {
            ...
            default:
                throw new GriffinException
                    .NotFoundException(GriffinExceptionMessage.NO_SUCH_JOB_ACTION);
        }
    }
    ...
    public void deleteJob(String name) throws SchedulerException {
        ...
        if (CollectionUtils.isEmpty(jobs)) {
            ...
            throw new GriffinException
                .NotFoundException(GriffinExceptionMessage.JOB_NAME_DOES_NOT_EXIST);
        }
        for (AbstractJob job : jobs) {
            JobEvent event = JobEvent.yieldJobEventBeforeRemoval(job);
            eventManager.notifyListeners(event);
    ...
    public JobInstanceBean findInstance(Long id) {
        ...
        if (bean == null) {
            ...
            throw new GriffinException
                .NotFoundException(GriffinExceptionMessage.INSTANCE_ID_DOES_NOT_EXIST);
        }
        return bean;
    }
    ...
    private JobOperator getJobOperator(AbstractJob job) {
        ...
        throw new GriffinException.BadRequestException
            (GriffinExceptionMessage.JOB_TYPE_DOES_NOT_SUPPORT);
    }
    ...
    private JobOperator getJobOperator(ProcessType type) {
        ...
        throw new GriffinException.BadRequestException
            (GriffinExceptionMessage.MEASURE_TYPE_DOES_NOT_SUPPORT);
    }
    ...
    TriggerKey getTriggerKeyIfValid(String qName, String qGroup) throws
        ...
        if (factory.getScheduler().checkExists(triggerKey)) {
            throw new GriffinException.ConflictException
                (GriffinExceptionMessage.QUARTZ_JOB_ALREADY_EXIST);
        }
        return triggerKey;
    }
    ...
    public Boolean isJobHealthy(Long jobId) {
        Pageable pageable = new PageRequest(0, 1, Sort.Direction.DESC);
        List<JobInstanceBean> instances = instanceRepo.findByJobId(jobId,
            pageable);
    ...
    public String getJobHdfsSinksPath(String jobName, long timestamp) {
        ...
        if (jobList.get(0).getType().toLowerCase().equals("batch")) {
            return getSinksPath(EnvConfig.ENV_BATCH)
                + "/" + jobName + "/" + timestamp + "";
        }

        return getSinksPath(EnvConfig.ENV_STREAMING)
            + "/" + jobName + "/" + timestamp + "";
    }
    ...
    public String triggerJobById(Long id) throws SchedulerException {
        ...
        LOGGER.info("triggerJobById: \n" + id);
        LOGGER.info("triggerJob.. job: \n" + job);
        validateJobExist(job);
        Scheduler scheduler = factory.getScheduler();
