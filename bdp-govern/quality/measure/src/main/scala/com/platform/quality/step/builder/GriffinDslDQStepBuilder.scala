...
package com.platform.quality.step.builder
...
import com.platform.quality.step.builder.dsl.parser.GriffinDslParser
import com.platform.quality.step.builder.dsl.transform
import com.platform.quality.step.builder.dsl.transform.Expr2DQSteps
  ...
  def buildSteps(context: DQContext, ruleParam: RuleParam): Seq[DQStep] = {
    ...
    try {
      ...
      if (result.successful) {
        val expr = result.get
        val expr2DQSteps = transform.Expr2DQSteps(context, expr, ruleParam.copy(outDfName = name))
