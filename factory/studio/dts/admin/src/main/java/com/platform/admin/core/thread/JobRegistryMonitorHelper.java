 ...
 * @author AllDataDC 2022-10-02 19:10:24
 */
		...
		registryThread = new Thread(() -> {
			while (!toStop) {
				try {
					...
					if (!toStop) {
						logger.error(">>>>>>>>>>> AllDataDC, job registry monitor thread error:{}", e);
					}
				}
				try {
			...
			logger.info(">>>>>>>>>>> AllDataDC, job registry monitor thread stop");
		});
		registryThread.setDaemon(true);
		registryThread.setName("AllDataDC, admin JobRegistryMonitorHelper");
