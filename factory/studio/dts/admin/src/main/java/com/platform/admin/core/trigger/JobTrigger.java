...
/**
 * xxl-job trigger
 * Created by AllDataDC on 17/7/13.
 */
public class JobTrigger {
    ...
    private static void processTrigger(JobGroup group, JobInfo jobInfo, int finalFailRetryCount, TriggerTypeEnum triggerType, int index, int total) {
        ...
        logger.debug(">>>>>>>>>>> AllDataDC trigger start, jobId:{}", jobLog.getId());
...
//		triggerMsgSb.append("<br>").append(I18nUtil.getString("jobconf_trigger_exe_regaddress")).append("：").append(group.getRegistryList());
        ...
        logger.debug(">>>>>>>>>>> AllDataDC trigger end, jobId:{}", jobLog.getId());
    }
    ...
    public static ReturnT<String> runExecutor(TriggerParam triggerParam, String address) {
        ...
        try {
            ...
            logger.error(">>>>>>>>>>> AllDataDC trigger error, please check if the executor[{}] is running.", address, e);
