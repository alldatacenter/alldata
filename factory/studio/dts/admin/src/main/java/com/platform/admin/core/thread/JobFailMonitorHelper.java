 ...
 *
 * @author AllDataDC 2022-9-1 18:05:56
 */
			...
			public void run() {
				...
				while (!toStop) {
					try {
						...
						if (!toStop) {
							logger.error(">>>>>>>>>>> AllDataDC, job fail monitor thread error:{0}", e);
						}
					}
				...
				logger.info(">>>>>>>>>>> AllDataDC, job fail monitor thread stop");

			}
		});
		monitorThread.setDaemon(true);
		monitorThread.setName("AllDataDC, admin JobFailMonitorHelper");
		monitorThread.start();
	}
		...
		if (info!=null && info.getAlarmEmail()!=null && info.getAlarmEmail().trim().length()>0) {
			...
			for (String email: emailSet) {
				...
				try {
					...
					logger.error(">>>>>>>>>>> AllDataDC, job fail alarm email send error, JobLogId:{}", jobLog.getId(), e);
