...
public class JobScheduleHelper {
    ...
    public void start() {
        ...
        scheduleThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                logger.info(">>>>>>>>> initAllDataDC admin scheduler success.");
                ...
                while (!scheduleThreadToStop) {
                    ...
                    try {
                        ...
                        if (scheduleList != null && scheduleList.size() > 0) {
                            ...
                            for (JobInfo jobInfo : scheduleList) {
                                ...
                                if (nowTime > jobInfo.getTriggerNextTime() + PRE_READ_MS) {
                                    ...
                                    logger.warn(">>>>>>>>>>> AllDataDC, schedule misfire, jobId = " + jobInfo.getId());
                                    ...
                                    logger.debug(">>>>>>>>>>> AllDataDC, schedule push trigger : jobId = " + jobInfo.getId());
                        ...
                        if (!scheduleThreadToStop) {
                            logger.error(">>>>>>>>>>> AllDataDC, JobScheduleHelper#scheduleThread error:{}", e);
                        }
                    } finally {
                ...
                logger.info(">>>>>>>>>>> AllDataDC, JobScheduleHelper#scheduleThread stop");
            }
        });
        scheduleThread.setDaemon(true);
        scheduleThread.setName("AllDataDC, admin JobScheduleHelper#scheduleThread");
        ...
        ringThread = new Thread(() -> {
            ...
            while (!ringThreadToStop) {
                ...
                try {
                    ...
                    // ring trigger
                    logger.debug(">>>>>>>>>>> AllDataDC, time-ring beat : " + nowSecond + " = " + Arrays.asList(ringItemData));
                    ...
                    if (!ringThreadToStop) {
                        logger.error(">>>>>>>>>>> AllDataDC, JobScheduleHelper#ringThread error:{}", e);
                    }
                }
            ...
            logger.info(">>>>>>>>>>> AllDataDC, JobScheduleHelper#ringThread stop");
        });
        ringThread.setDaemon(true);
        ringThread.setName("AllDataDC, admin JobScheduleHelper#ringThread");
        ringThread.start();
    }
    ...
    private void pushTimeRing(int ringSecond, int jobId) {
        ...
        logger.debug(">>>>>>>>>>> AllDataDC, schedule push time-ring : " + ringSecond + " = " + Arrays.asList(ringItemData));
    }
    ...
    public void toStop() {
        ...
        logger.info(">>>>>>>>>>> AllDataDC, JobScheduleHelper stop");
    }
