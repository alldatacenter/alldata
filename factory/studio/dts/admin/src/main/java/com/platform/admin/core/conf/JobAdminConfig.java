...
public class JobAdminConfig implements InitializingBean, DisposableBean {
    ...
    // conf
    @Value("${studio.job.i18n}")
    private String i18n;

    @Value("${studio.job.accessToken}")
    private String accessToken;

    @Value("${spring.mail.username}")
    private String emailUserName;

    @Value("${studio.job.triggerpool.fast.max}")
    private int triggerPoolFastMax;

    @Value("${studio.job.triggerpool.slow.max}")
    private int triggerPoolSlowMax;

    @Value("${studio.job.logretentiondays}")
