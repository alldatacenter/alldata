...
usage(){
  ...
  printf "\t%-10s  %-10s  %-2s \n" --server "server-name" "Name of studio-admin server"
...
BIN=`abs_path`
SERVER_NAME_SIMPLE=${SERVER_NAME/studio-/}
...
if [ "x${SQL_SOURCE_PATH}" != "x" ] && [ -f "${SQL_SOURCE_PATH}" ]; then
   ...
   if [ $? == 0 ]; then
      ...
      if [ $? == 0 ]; then
        ...
        read -p "Please input the db name(default: studioweb)" DATABASE
