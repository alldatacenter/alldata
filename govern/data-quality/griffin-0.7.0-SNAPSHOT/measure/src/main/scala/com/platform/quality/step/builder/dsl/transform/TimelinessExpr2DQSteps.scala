...
import com.platform.quality.step.builder.dsl.expr.{Expr, TimelinessClause}
import com.platform.quality.step.builder.dsl.transform.analyzer.TimelinessAnalyzer
...
import com.platform.quality.utils.TimeUtil
...
case class TimelinessExpr2DQSteps(context: DQContext, expr: Expr, ruleParam: RuleParam)
  ...
  def getDQSteps: Seq[DQStep] = {
    ...
    if (!context.runTimeTableRegister.existsTable(sourceName)) {
      ...
      val analyzer = analyzer.TimelinessAnalyzer(timelinessExpr, sourceName)
      ...
      val metricWriteStep = {
        ...
        write.MetricWriteStep(mwName, metricTableName, flattenType)
      }
      val metricTransStep =
