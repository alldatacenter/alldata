...
import com.platform.quality.configuration.dqdefinition.{RuleErrorConfParam, RuleParam}
import com.platform.quality.step.builder.ConstantColumns
import com.platform.quality.step.builder.dsl.expr.{CompletenessClause, Expr}
import com.platform.quality.step.builder.dsl.transform.analyzer.CompletenessAnalyzer
...
import com.platform.quality.step.write.{MetricWriteStep, RecordWriteStep}
...
case class CompletenessExpr2DQSteps(context: DQContext, expr: Expr, ruleParam: RuleParam)
  ...
  def getDQSteps: Seq[DQStep] = {
    ...
    if (!context.runTimeTableRegister.existsTable(sourceName)) {
      ...
      val analyzer = analyzer.CompletenessAnalyzer(completenessExpr, sourceName)
      ...
      val completeWriteStep = {
        ...
        write.MetricWriteStep(mwName, completeTableName, flattenType)
      }
