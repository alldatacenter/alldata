 ...
 * @desc:
 */
public abstract class FileInputFormatPlugin<OT extends Row> extends InputFormatPlugin<OT, FileInputSplit> {
...
// -------------------------------------- Constants -------------------------------------------
  ...
  /**
   * Cached Statistics to avoid redundant computation
   */
  private transient FileBaseStatistics cachedStatistics;

  public FileInputFormatPlugin() {
  }

  protected FileInputFormatPlugin(Path filePath) {
    if (filePath != null) {
   ...
   */
  @Override
  public FileInputSplit[] createSplits(int minNumSplits) throws IOException {
    if (minNumSplits < 1) {
  ...
  public void open(FileInputSplit fileSplit) throws IOException {
    ...
    final FileInputFormat.InputSplitOpenThread isot = new FileInputFormat.InputSplitOpenThread(fileSplit, this.openTimeout);
    isot.start();
   ...
   * When overriding this method, also consider adapting {@link FileInputFormat#} if your
