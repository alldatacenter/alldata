...
import com.bytedance.bitsail.connector.legacy.hudi.sink.StreamWriteOperator;
import com.bytedance.bitsail.connector.legacy.hudi.sink.append.AppendWriteOperator;
import com.bytedance.bitsail.connector.legacy.hudi.sink.bootstrap.BootstrapOperator;
import com.bytedance.bitsail.connector.legacy.hudi.sink.bootstrap.batch.BatchBootstrapOperator;
import com.bytedance.bitsail.connector.legacy.hudi.sink.bucket.BucketBulkInsertWriterHelper;
import com.bytedance.bitsail.connector.legacy.hudi.sink.bucket.BucketStreamWriteOperator;
...
//      LogicalTypeUtils.toInternalConversionClass();
//      TypeInformation.of()

      Map<String, String> bucketIdToFileId = new HashMap<>();
      dataStream = dataStream.partitionCustom(partitioner, keyGen::getHoodieKey)
          .map(record -> BucketBulkInsertWriterHelper.rowWithFileId(bucketIdToFileId, keyGen, record, indexKeys, numBuckets))
...
//        dataStream = dataStream.transform("file_sorter", TypeInformation.of(RowData.class), sortOperatorGen.createSortOperator())
//            .setParallelism(conf.getInteger(FlinkOptions.WRITE_TASKS)); // same parallelism as write task to avoid shuffle
