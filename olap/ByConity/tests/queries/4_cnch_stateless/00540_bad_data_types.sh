...
$CLICKHOUSE_CLIENT --query="SELECT CAST(0 AS DateTime('Etc/UTC'))";
...
($CLICKHOUSE_CLIENT --query="SELECT * FROM system.one WHERE 256 SETTINGS ENABLE_OPTIMIZER = 0" 2>/dev/null && echo "Expected failure") || true;
($CLICKHOUSE_CLIENT --query="SELECT * FROM system.one WHERE -1 SETTINGS ENABLE_OPTIMIZER = 0" 2>/dev/null && echo "Expected failure") || true;
($CLICKHOUSE_CLIENT --query="SELECT * FROM system.one WHERE CAST(256 AS Nullable(UInt16)) SETTINGS ENABLE_OPTIMIZER = 0" 2>/dev/null && echo "Expected failure") || true;
