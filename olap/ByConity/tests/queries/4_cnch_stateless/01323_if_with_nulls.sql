SELECT if(1 = 0, toNullable(toUInt8(0)), NULL) AS x, toTypeName(if(1 = 0, toNullable(toUInt8(0)), NULL));
SELECT if(1 = 1, toNullable(toUInt8(0)), NULL) AS x, toTypeName(if(1 = 1, toNullable(toUInt8(0)), NULL));
SELECT if(1 = 1, NULL, toNullable(toUInt8(0))) AS x, toTypeName(if(1 = 1, NULL, toNullable(toUInt8(0))));
SELECT if(1 = 0, NULL, toNullable(toUInt8(0))) AS x, toTypeName(if(1 = 0, NULL, toNullable(toUInt8(0))));

SELECT if(toUInt8(0), NULL, toNullable(toUInt8(0))) AS x, if(if(toUInt8(0), NULL, toNullable(toUInt8(0))) = 0, 'ok', 'fail');
SELECT if(toUInt8(1), NULL, toNullable(toUInt8(0))) AS x, if(if(toUInt8(1), NULL, toNullable(toUInt8(0))) = 0, 'fail', 'ok');
SELECT if(toUInt8(1), toNullable(toUInt8(0)), NULL) AS x, if(if(toUInt8(1), toNullable(toUInt8(0)), NULL) = 0, 'ok', 'fail');
SELECT if(toUInt8(0), toNullable(toUInt8(0)), NULL) AS x, if(if(toUInt8(0), toNullable(toUInt8(0)), NULL) = 0, 'fail', 'ok');
...
CREATE TABLE test_nullable_float_issue7347 (ne UInt64,test Nullable(Float64)) ENGINE = CnchMergeTree() PRIMARY KEY (ne) ORDER BY (ne);
...
SELECT test, toTypeName(test), IF(test = 0, 1, 0) FROM (SELECT materialize(CAST(NULL, 'Nullable(Float64)')) AS test);
