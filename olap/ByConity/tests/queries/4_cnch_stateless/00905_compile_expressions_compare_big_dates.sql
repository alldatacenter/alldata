...
CREATE TABLE foo_c(d DateTime) ENGINE = CnchMergeTree ORDER BY d;
...
SELECT toDate(d) AS dd FROM foo_c WHERE (toDate(d) >= '2019-02-06') AND (toDate(d) <= toDate('2019-08-09')) GROUP BY toDate(d) ORDER BY dd;
SELECT toDate(d) FROM foo_c WHERE (toDate(d) > toDate('2019-02-10')) AND (toDate(d) <= toDate('2022-01-01')) ORDER BY d;
DROP TABLE IF EXISTS foo_c;
