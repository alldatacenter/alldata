...
CREATE DATABASE IF NOT EXISTS `test_00565`
EOF

$CLICKHOUSE_CLIENT <<'EOF'
CREATE TABLE `test_00565`.`test_log` (
    date Date,
    datetime DateTime,
    path String,
    gtid String,
...
QUERY='INSERT INTO `test_00565`.`test_log`(`date`, `datetime`, `path`, `gtid`, `query_serial`, `row_serial`,
...
echo 'Count:'
$CLICKHOUSE_CLIENT --query 'select count() from test_00565.test_log'
$CLICKHOUSE_CLIENT --query 'DROP TABLE test_00565.test_log'
