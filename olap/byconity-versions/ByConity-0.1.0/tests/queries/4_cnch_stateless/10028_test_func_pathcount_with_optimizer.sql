...
PARTITION BY (toStartOfDay(`event_date`), `app_id`)
PRIMARY KEY (`app_id`, `event`, `event_date`, `hash_uid`, `user_unique_id`)
ORDER BY (`app_id`, `event`, `event_date`, `hash_uid`, `user_unique_id`)
    ...
    (
        ...
        FROM
            (
                ...
                FROM tob_apps AS et
                WHERE (app_id = 173715) AND ((event = 'goods_pay_success') OR (event = 'goods_pay_success') OR (event = 'goods_pay_success')) AND ((event_date >= '2022-06-22') AND (event_date <= '2022-06-28') AND (multiIf(server_time < 1609948800, server_time, time > 2000000000, toUInt32(time / 1000), time) >= 1655827200) AND (multiIf(server_time < 1609948800, server_time, time > 2000000000, toUInt32(time / 1000), time) <= 1656431999))
                GROUP BY hash_uid
            )
        GROUP BY path
            SETTINGS distributed_perfect_shard = 1
