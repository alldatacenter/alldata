...
import com.bytedance.bitsail.connector.legacy.hudi.sink.event.CommitAckEvent;
...
import com.bytedance.bitsail.connector.legacy.hudi.sink.utils.HiveSyncContext;
...
import static com.bytedance.bitsail.connector.legacy.hudi.util.StreamerUtil.initTableIfNotExists;
...
//  }
  ...
  private void sendCommitAckEvents(long checkpointId) {
    CompletableFuture<?>[] futures = IntStream.range(0, this.parallelism)
        .mapToObj(taskID -> {
          try {
            return this.context.sendEvent(CommitAckEvent.getInstance(checkpointId), taskID);
          } catch (TaskNotRunningException e) {
            //In LazyInitializedCoordinatorContext, never reach it. All exceptions are wrapped in CompletableFuture.
            LOG.warn("Error while sending commit ack event to task [" + taskID + "]", e);
            return null;
          }
