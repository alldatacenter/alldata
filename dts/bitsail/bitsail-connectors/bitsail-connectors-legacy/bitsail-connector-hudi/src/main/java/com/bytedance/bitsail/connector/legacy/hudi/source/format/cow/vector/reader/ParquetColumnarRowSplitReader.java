...
package com.bytedance.bitsail.connector.legacy.hudi.source.format.cow.vector.reader;
...
public class ParquetColumnarRowSplitReader implements Closeable {
  ...
  private WritableColumnVector[] createWritableVectors() {
    ...
    for (int i = 0; i < requestedTypes.length; i++) {
      columns[i] = ParquetSplitReaderUtil.createWritableColumnVector(
          batchSize,
          requestedTypes[i],
          types.get(i),
          descriptors);
  ...
  private ColumnVector[] createReadableVectors() {
    ...
    for (int i = 0; i < writableVectors.length; i++) {
      vectors[i] = requestedTypes[i].getTypeRoot() == LogicalTypeRoot.DECIMAL ? new ParquetDecimalVector(writableVectors[i])
          : writableVectors[i];
    }
    return vectors;
  }
