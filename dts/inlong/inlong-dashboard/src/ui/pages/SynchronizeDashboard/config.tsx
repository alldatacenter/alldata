...
export const dashCardList = [
  ...
  {
    desc: i18n.t('pages.SynchronizeDashboard.config.WaitAssignCount'),
    dataIndex: 'waitAssignCount',
    icon: <DashToBeAssigned />,
  },
  {
    desc: i18n.t('pages.SynchronizeDashboard.config.WaitApproveCount'),
    dataIndex: 'waitApproveCount',
    icon: <DashPending />,
  },
  {
    desc: i18n.t('pages.SynchronizeDashboard.config.Reject'),
...
export const useColumns = ({ onDelete, openModal, onRestart, onStop }) => {
  ...
  const { Entity } = useLoadMeta<SyncMetaType>('sync', defaultValue);

  const entityColumns = useMemo(() => {
    return Entity ? new Entity().renderList() : [];
  }, [Entity]);

  const genCreateUrl = record => `/sync/create/${record.inlongGroupId}`;
  const genDetailUrl = record =>
    [0, 100].includes(record.status)
      ? genCreateUrl(record)
      : `/sync/detail/${record.inlongGroupId}`;
          ...
          <>
            ...
            {record?.status && record?.status === 130 && (
              <Button type="link" onClick={() => onStop(record)}>
                {i18n.t('pages.SynchronizeDashboard.config.Stop')}
              </Button>
            )}
            {record?.status && (record?.status === 120 || record?.status === 130) && (
              <Button type="link" onClick={() => openModal(record)}>
                {i18n.t('pages.SynchronizeDashboard.config.ExecuteLog')}
              </Button>
            )}
          </>
        ),
