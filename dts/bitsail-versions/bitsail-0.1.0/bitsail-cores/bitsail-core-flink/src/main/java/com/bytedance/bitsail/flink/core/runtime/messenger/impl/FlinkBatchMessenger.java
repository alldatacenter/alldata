 ...
 * Created 2022/7/14
 */
public class FlinkBatchMessenger extends AbstractBatchMessenger<Row> {

  public FlinkBatchMessenger(MessengerContext messengerContext) {
    super(messengerContext);
  }

  @Override
  public void addSuccessRecord(Row message) {
    successRecordsCounter.inc();
    successBytesCounter.inc(RowUtil.getRowBytesSize(message));

    successRecordsMeter.markEvent();
    successBytesMeter.markEvent(RowUtil.getRowBytesSize(message));
  }

  @Override
  public void addFailedRecord(Row message, Throwable throwable) {
